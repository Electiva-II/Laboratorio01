...
datos$Fac_Odor <- ifelse(datos$odor %in% c("almond","anise"),
                         "pleasant",
                         "strong")
...
pred_naive <- rep(clase_mayoritaria, length(y_test))

acc_naive <- mean(pred_naive == y_test)
...
train$class_bin <- ifelse(train$class == "p", 1, 0)
test$class_bin  <- ifelse(test$class == "p", 1, 0)
...
# probabilidades
prob_pred <- predict(modelo_glm, test, type = "response")

# clases finales
pred_glm <- ifelse(prob_pred > 0.5, 1, 0)

acc_glm <- mean(pred_glm == test$class_bin)
...
resultados <- data.frame(
  Modelo = c("Ingenuo (Baseline)", "Regresión Logística (GLM)"),
  Accuracy = c(acc_naive, acc_glm)
)
